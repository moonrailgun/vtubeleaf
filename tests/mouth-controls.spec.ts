...
for (const load of ['startup', 'scene'] as const) {
  test(`${load} loading repairs old VTS mouth neutral and saves it without reapplying other settings`, async ({
    ...
    await page.route('**/src/main.tsx*', async (route) => {
      ...
      const startup = { ...model, id: 'startup', path: '/startup/Haru.model3.json', name: 'Startup' };
      mockIPC(async (cmd, args) => {
        ...
        if (cmd === 'list_models') return { models: [model, startup], directory: '/mouth', errors: [] };
        if (cmd === 'load_model') return args.path === model.path ? model : startup;
    ...
    await page.addInitScript(
        ...
        localStorage.setItem(
          ...
          JSON.stringify(
            load === 'startup'
              ? profile
              : {
                  modelPath: '/startup/Haru.model3.json',
                  autoCheckUpdates: false,
                  profiles: { [modelPath]: profile },
                  scenes: [
                    {
                      id: 'mouth',
                      name: 'Mouth scene',
      ...
      { modelPath: model.path, load },
    );
    await page.goto('/');
    if (load === 'scene') {
      await expect(page.locator('#model-name')).toHaveText('Startup');
      await expect(page.locator('#start')).toBeEnabled();
      await page.getByRole('button', { name: '画面', exact: true }).click();
      await page.locator('#saved-scene').click();
      await page.getByRole('option', { name: 'Mouth scene', exact: true }).click();
